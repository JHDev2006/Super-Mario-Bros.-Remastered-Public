...
func enter(msg := {}) -> void:
	...
	player.velocity.y = -player.POUND_HANG_SPEED
	player.is_pounding = true
	
	ground_collision = player.find_child("GroundBlockCollision")
	hang_time = player.POUND_HANG_TIME
	grounded_time = 0.0
	is_hanging = true
	
	AudioManager.play_sfx("ground_pound", player.global_position)

func physics_update(_delta: float) -> void:
	player.play_animation("GroundPound")
	
	if is_hanging:
		player.velocity.y = move_toward(player.velocity.y, 0, player.POUND_HANG_DECEL * _delta)
		hang_time -= _delta
		if hang_time <= 0:
			is_hanging = false
	else:
		player.velocity.y = player.POUND_FALL_SPEED
				...
				if i is Block:
					if is_big:
						if i is BrickBlock and not hit_block and not i.item:
							AudioManager.play_sfx("cannon", player.global_position)
							hit_block = true
